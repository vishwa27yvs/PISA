...
def load_data_subset(batch_size,workers,dataset, data_target_dir, labels_per_class=100, valid_labels_per_class = 500, mixup_alpha=1):
    ...
    if dataset == 'tiny-imagenet-200':
        ...
        # labelled = torch.utils.data.DataLoader(train_data, batch_size=96,shuffle=True, num_workers=workers, pin_memory=True)
        # validation = None
        # unlabelled = None
        # test = torch.utils.data.DataLoader(test_data, batch_size=256, shuffle=False, num_workers=workers, pin_memory=True)
        labelled = torch.utils.data.DataLoader(train_data, batch_size=96, sampler = train_sampler, shuffle=False, num_workers=workers, pin_memory=True)
